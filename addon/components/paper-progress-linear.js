...
export default Component.extend(ColorMixin, {
  ...
  bar1Style: computed('clampedBufferValue', function() {
    return htmlSafe(`${this.constants.CSS.TRANSFORM}: ${makeTransform(this.clampedBufferValue)}`);
  }),
  ...
  bar2Style: computed('clampedValue', 'mode', function() {
    ...
    return htmlSafe(`${this.constants.CSS.TRANSFORM}: ${makeTransform(this.clampedValue)}`);
  }),
